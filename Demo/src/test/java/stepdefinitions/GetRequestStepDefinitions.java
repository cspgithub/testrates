...
public class GetRequestStepDefinitions extends TestBase {
	...
	public GetRequestStepDefinitions(TestContextAPI testContext) {
		this.testContext = testContext;// <-- This is where pico-containers starts working
	}

	@Given("Go rest API is up and running")
	public void go_rest_API_is_up_and_running() {
		// <-- Linking your driver object reference from the point where it is
		// instantiated , i.e the hooks class
	...
	@Then("API returned the status code as {string}")
	public void api_returned_the_status_code_as(String statusMsg) {
		testContext.resp.then().assertThat().statusLine(statusMsg);

		testContext.scn.write("Validated the status code,status code is : " + statusMsg + "<br>");
	}

	@Then("API returned the status code as {int}")
	public void api_returned_the_status_code_as(Integer statusMsg1) {
		testContext.resp.then().assertThat().statusCode(statusMsg1);

		testContext.scn.write("Validated the status code,status code is : " + statusMsg1 + "<br>");
	}
	...
	public void api_should_return_current_date_rates() {
		testContext.resp.then().assertThat().body("date", equalTo(locatDate()));
	}
